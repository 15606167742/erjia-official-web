<template>
	<div class="top-menu">
		...
		<div class="tabs">
			<div class="tab" :class="{ active: $route.path == tab.path }" :key="tab.path" v-for="tab in tabs" @click="gotoPath(tab.path)">{{ tab.text }}</div>
		</div>
		...
		<el-drawer :visible="showMenu" direction="rtl" :with-header="false" :show-close="false" :size="200" :modal-append-to-body="false" :before-close="handleClose">
			<el-menu :default-active="$route.path" background-color="#FFF" text-color="#333" active-text-color="var(--color-t-active)" @select="gotoPath">
